...
class FileService:
    ...
    files_db: ExpiringDict = ExpiringDict(
        max_len=100, max_age_seconds=FileSystem.FILE_EXPIRATION
    )
    ...
    async def create_upload_link(
        cls, file_data: str, file_name: str, server_token: Optional[str] = None
    ):
        file_bytes = base64.b64decode(file_data)
        if not file_name.lower().endswith(
            (".wld", ".twld", ".plr", ".tplr", ".map", ".tmap", ".zip")
        ):
            return {"success": False, "message": "无效文件名"}
        ...
        if file_name.lower().endswith(".zip"):
            if not await cls.validate_zip_content(file_bytes):
                ...
                return {"success": False, "message": "压缩包内含有无效文件名"}
