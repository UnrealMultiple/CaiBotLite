...
minecart = Image.open("img/Item/item_2343.png").convert('RGBA')
...
def get_bag_png(name: str, inv: list['()'], buffs: list, enhances: list, life: str, mana: str,
    ...
    bg_img = Image.open("img/Background_4.png").convert('RGBA')
    ...
    def draw_item(x: int, y: int, item_id: int, stack: int):
        try:
            # 从缓存中获取图片
            item = image_cache.get(f"item_{item_id}")
            if item is None:
    ...
    def draw_buff(x: int, y: int, buff_id: int):
        try:
            if buff_id == 0:
                return
            # 从缓存中获取图片
            buff = image_cache.get(f"buff_{buff_id}")
            if buff is None:
                raise ValueError(f"Buff image with ID {buff_id} not found in cache.")
    ...
    if quests_completed >= 50:
        bg_img.paste(golden_fishing_rod, (915 + 50 + 70, 120 + 280 + 30 + 30 + 30 + 40 + 40), mask=agolden_fishing_rod)
    ...
    if len(economic) != 0 and economic['Coins'] != "":
        ...
        if economic['LevelName'] != "":
            bg_img.paste(warrior_emblem, (915 + 50 + 70, 250 + 120 + 280 + 30 + 30 + 22 + 40 * h), mask=awarrior_emblem)
...
if __name__ == '__main__':
    data = {"type": "lookbag", "name": "Cai", "exist": 1, "life": "9999/9999", "mana": "20/20", "quests_completed": 0,
            "inventory": [[3509, 1], [5011, 1], [3506, 1], [8, 5], [183, 7], [3509, 1], [706, 10], [4444, 1],
                          ...
                          [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3],
                          [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3],
                          [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [2, 3], [1, 0]], "buffs": [1,2,1,2], "enhances": [1,2,3,2],
            "economic": {
